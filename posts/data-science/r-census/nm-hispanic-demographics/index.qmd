...
us_hisp_pct |> 
  ...
  scale_fill_viridis_c(labels = scales::percent, limits = c(0,.5)) +
  theme_void() +
...
The counties with the largest percentage of Hispanics:
...
Hispanic percentage in the most populous counties:
...
```{r}
#| eval: false
saveRDS(hispanic_us, "data/hispanic_us.rds")
```

```{r}
hispanic_us <- readRDS("data/hispanic_us.rds")
```
